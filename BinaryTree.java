...
public class BinaryTree<E> {
    ...
    public String toStringHelper(String indent, String childIndent) {
        ...
        if (hasLeft()) {
            //not null left child
            res += left.toStringHelper(childIndent + "â”œâ”€â”€ ", childIndent + "â”‚   ");
        }
        else if (!isLeaf()){
            //null left child
            res += childIndent + "â”œâ”€â”€ null\n";
        }
        if (hasRight()) {
            //not null right child
            res += right.toStringHelper(childIndent + "â””â”€â”€ ", childIndent + "    ");
        }
        else if (!isLeaf()){
            //null right child
            res += childIndent + "â””â”€â”€ null\n";
        }
        return res;
    }
    ...
    private void copyBinaryTree(BinaryTree<E> givenTree, BinaryTree<E> copy, int depth){
        if(depth == 0) return;
        copy = givenTree;
        if(givenTree.hasLeft()) copyBinaryTree(givenTree.getLeft(), copy.getLeft(), depth - 1);
        if(givenTree.hasRight()) copyBinaryTree(givenTree.getRight(), copy.getLeft(), depth - 1);


    }
    public BinaryTree<E> copy(BinaryTree<E> tree, int depth){
        BinaryTree<E> copyTree = new BinaryTree<E>(null);
        copyBinaryTree(tree, copyTree, depth);
        return copyTree;
    }
}